#ifndef zikzak_inappWEBVIEW_PLUGIN_LOG_UTIL_H_
#define zikzak_inappWEBVIEW_PLUGIN_LOG_UTIL_H_
...
namespace zikzak_inappwebview_plugin
{
  ...
  static inline void debugLog(const std::basic_string<T>& msg, const bool& isError = false, const std::string& filename = "", const int& line = 0)
  {
    ...
    if (!filename.empty() && line > 0) {
      auto filenameSplit = split(filename, std::string{ "\\zikzak_inappwebview_windows\\" });
      std::string reduceFilenamePath = filenameSplit.size() > 0 ? "zikzak_inappwebview_windows\\" + filenameSplit.back() : filename;
...
#endif //zikzak_inappWEBVIEW_PLUGIN_LOG_UTIL_H_
