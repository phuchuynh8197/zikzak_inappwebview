...
namespace zikzak_inappwebview_plugin
{
  ...
  CustomPlatformView::CustomPlatformView(flutter::BinaryMessenger* messenger,
    ...
    HWND hwnd,
    std::unique_ptr<zikzak_inappwebview_plugin::InAppWebView> webView)
    : hwnd_(hwnd), view(std::move(webView)), texture_registrar_(texture_registrar)
  {
    ...
    const auto method_channel_name = "wtf.zikzak/custom_platform_view_" + std::to_string(texture_id_);
    method_channel_ =
    ...
    const auto event_channel_name = "wtf.zikzak/custom_platform_view_" + std::to_string(texture_id_) + "_events";
    event_channel_ =
    ...
    if (method_name.compare(kMethodSetPointerUpdate) == 0) {
      ...
      if (pointer && event && x && y && size && pressure && view) {
        view->setPointerUpdate(*pointer,
          static_cast<zikzak_inappwebview_plugin::InAppWebViewPointerEventKind>(*event),
          *x, *y, *size, *pressure);
        return result->Success();
      }
      return result->Error(kErrorInvalidArgs);
      ...
      if (button != map.end() && isDown != map.end()) {
        ...
        if (buttonValue && isDownValue && view) {
          view->setPointerButtonState(
            static_cast<zikzak_inappwebview_plugin::InAppWebViewPointerButton>(*buttonValue), *isDownValue);
          return result->Success();
        }
      }
      return result->Error(kErrorInvalidArgs);
